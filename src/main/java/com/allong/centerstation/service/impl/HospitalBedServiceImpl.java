...
import com.allong.centerstation.domain.entity.*;
...
public class HospitalBedServiceImpl extends ServiceImpl<HospitalBedMapper, HospitalBed> implements HospitalBedService {
    ...
    @Autowired
    private BedAlarmConfigService configService;
    ...
    @Override
    public List<BedDetail> listBedEnableAlarm() {
        //如果是管理员，则显示全部医院
        List<BedDetail> bedDetails ;
        String username = SecurityUtils.getCurrentUsername();
        if (username == null || username.equals("")) {//表示未登录，注解界面获取
            ...
            //判断床位是否可报警
            List<BedAlarmConfig> bedAlarmConfigs =configService.list();
            for(BedDetail item:bedDetails){
                for(BedAlarmConfig bed:bedAlarmConfigs){
                    if(item.getId().equals(bed.getBedId()) && bed.getEnable()){
